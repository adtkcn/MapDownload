<template>
  ...
  <div class="box-controls">
    ...
    <map-key :visible="setVisible" @hide="showSet(false)" />
  </div>
  <ProgressControl />
  <tips />
</template>
  ...
  methods: {
    ...
    showSave(showMsg = true) {
      ...
      if (!this.downloadExtent) {
        if (showMsg)
          window.$message.warning("获取下载范围错误，请重新绘制下载范围");
        return false;
      }
      const { tileLayer, maxZoom, minZoom, projection } =
        map.getBaseMapConfig();
    ...
    showSet(val) {
      ...
      if (
        !val &&
        this._currentLayer &&
        (this._currentLayer.parent === "Tdt" ||
          this._currentLayer.parent === "Mapbox")
      ) {
