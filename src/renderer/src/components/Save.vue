  ...
  >
    ...
    <div class="dialog-content">
      ...
      <div class="item">
        ...
        <div class="value">
          <n-select v-model:value="imageType" :options="imageTypeList" />
        </div>
      </div>
    </div>
    <template #action>
...
export default defineComponent({
  ...
  methods: {
    ...
    ok() {
      ...
      if (!this.savePath) {
        return $message.warning('请选择保存目录')
      }
      if (!this.maxZoom) {
        return $message.warning('请输入最大层级')
      }
      if (!this.minZoom) {
        return $message.warning('请输入最小层级')
      }
      const minZoom = parseInt(this.minZoom)
      const maxZoom = parseInt(this.maxZoom)
      if (isNaN(minZoom) || isNaN(maxZoom)) {
        return $message.warning('层级格式错误，请输入非负整数')
      }
      if (minZoom > maxZoom || minZoom < this.limitMinZoom || maxZoom > this.limitMaxZoom) {
        return $message.warning('层级格式错误')
      }
      ...
      const param = {
        ...
        extent: toRaw(this.downloadExtent),
