 ...
 * @returns
 */
maptalks.TileLayer.prototype.downloadCascadeTiles = function (
  z,
  downloadOption
) {
  console.log("downloadCascadeTiles", z, downloadOption);
  ...
  if (
    ...
    const containerExtent =
      pitch <= cascadePitch1
        ? mapExtent
        : new PointExtent(0, map.height - visualHeight1, map.width, map.height);
    this.downloadTiles(
      tileZoom,
      containerExtent,
      2,
  ...
  const extent0 = new PointExtent(
    0,
    map.height - visualHeight0,
    map.width,
    map.height
  );
  this.downloadTiles(tileZoom, extent0, 0, downloadOption);
  ...
  if (pitch > cascadePitch1) {
    ...
    const extent1 = new PointExtent(
      0,
      map.height - visualHeight1,
      map.width,
      cascadeHeight
    );
    this.downloadTiles(
      tileZoom - cascadeLevels,
      extent1,
      1,
 ...
 *
 * @param {*} tileZoom 缩放层级
 * @param {object} containerExtent {xmax: 759.1875
xmin: 0
ymax: 791.1875
ymin: 0
_clazz: ƒ Point2()
 ...
 * @returns
 */
maptalks.TileLayer.prototype.downloadTiles = async function (
  tileZoom,
  containerExtent,
  cascadeLevel,
  ...
  if (cascadeLevel < 2) {
    ...
    frustumMatrix =
      cascadeLevel === 0
        ? map.cascadeFrustumMatrix0
        : cascadeLevel === 1
        ? map.cascadeFrustumMatrix1
        : map.projViewMatrix;
  }
  ...
  if (maskExtent) {
    ...
    containerExtent = intersection.convertTo((c) =>
      map._pointToContainerPoint(c, undefined, 0, TEMP_POINT)
    );
  }
  //Get description of center tile including left and top offset
  const prjCenter = map._containerPointToPrj(
    containerExtent.getCenter(),
    TEMP_POINT0
  );
  ...
  if (hasOffset) {
    c = this._project(
      map._pointToPrj(centerPoint._add(offset), zoom, TEMP_POINT1),
      TEMP_POINT1
    );
  } else {
  ...
  const pmin = this._project(
    map._pointToPrj(TEMP_POINT2._add(offset), zoom, TEMP_POINT2),
    TEMP_POINT2
  );
  const pmax = this._project(
    map._pointToPrj(TEMP_POINT3._add(offset), zoom, TEMP_POINT3),
    TEMP_POINT3
  );
  ...
  const renderer = this.getRenderer() || undefined,
  ...
  for (let i = -top; i <= bottom; i++) {
    ...
    while (j >= leftVisitEnd && j <= right) {
      const idx = tileConfig.getNeighorTileIndex(
        centerTile.idx,
        centerTile.idy,
        j,
        i,
      ...
      if (!this._hasOwnSR) {
        ...
        if (!this._hasOwnSR) {
          ...
          pp = map._prjToPoint(
            this._unproject(pse, TEMP_POINT3),
            z,
            TEMP_POINT3
          );
        }
      ...
      const tileExtent =
        (tileInfo && tileInfo.extent2d) ||
        new PointExtent(p.x, p.y - height, p.x + width, p.y);
      // if (hasOffset) {
      ...
      if (
        allCount <= 4 ||
        rightVisitEnd ||
        this._isTileInExtent(frustumMatrix, tileExtent, offset, glScale)
      ) {
        ...
        if (cascadeLevel === 0) {
          ...
          downloadImage(tiles[0], downloadOption);
        } else {
          if (!tileInfo) {
            tileInfo = {
          ...
          downloadImage(tileInfo, downloadOption);
